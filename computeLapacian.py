...
def computeLaplacian(img, eps = 10**(-7), win_rad=1):
    ...
    L = scipy.sparse.csc_matrix((nz_indsVal, (nz_indsRow, nz_indsCol)), shape=(h*w, h*w))

    return L
    #sumA = np.sum(np.asmatrix(L), 1)
    #return scipy.sparse.spdiags(sumA, 0, w*h, w*h) - L
    #sumL = np.sum(L, 1)
    #return scipy.sparse.spdiags(sumL, 0, h*w, h*w).toarray()
...
if __name__ == "__main__":
    ...
    img = scipy.misc.imread('../examples/input/in1.png')
    print (computeLaplacian(img))
